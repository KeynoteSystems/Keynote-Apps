...
class DailyController extends Zend_Controller_Action
{
	...
	public function init()
	{
		...
		/**
		 * create a variable that holds the current date and time
		 */ 
		...
		foreach ($slotData->product as $a) {
			if ($a->name == $product) {
				foreach ($a->slot as $b) {
					...
					if ($b->slot_id != 1091870) {   			// ignore the slot 1091970, the slot for Keynote Business 40 index

						$endDate = $b->end_date;			
						if ($endDate > $cDate) {				// only include slots that are still current: end_date > current date
							
							$nPages = explode(',', $b->pages); 	// separate the individual pages in the slot. (multiple pages are separated by a ,)
							$daysLeft = strtotime($endDate) - strtotime($cDate);
							
							$slotIds[] = $b->slot_id; 			// add this slot_id to the array of selected slotIds
							
							for ($i = 0; $i < count($nPages); $i++) {  // create a list of transaction pages for each slot
								$r = $i + 1;
	...
	/**
	 * Create Mail Action Handler that handles the request /daily/createmail/product/{product_code}.
	 * 
	 * Intended for debugging purposes, creates an email/printer friendly view of the product report.
	 */
	...
	/**
	 * Send Mail Action Handler that handles the request /daily/sendmail/prodcut/{product_code}.
	 * 
	 * Intended for debugging purposes: send the result of the create mail action by email.
	 * Contains hardcoded values for the URL of the `createmail` action as well
	 * as the mail server. 
	 */
	public function sendmailAction()
	{
		// disable the layout as this function simply send the mail
		$this->_helper->layout->disableLayout();

		// No rendering is required
		$this->_helper->viewRenderer->setNoRender();

		// Make a curl call to the createmail action 
		$crl = curl_init();
		...
		// Execute the curl call and capture the response
		$str = curl_exec($crl);
		...
		// Replace the commented code below with your account details
		...
		// Set the mail details: replace with appropriate to and from accounts
		$mail = new Zend_Mail();
